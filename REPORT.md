...
Nếu bạn **đã xem Summary tự đánh giá trên GitHub Actions hoặc tự chạy script**, ghi ngắn một kết quả liên quan lỗi vừa sửa (ví dụ task, metric trước/sau nếu có): Sau khi tách đối tượng, script `inspect_submissions.py` nhận diện đủ 16 annotations riêng biệt; chạy kiểm tra tự đánh giá trên `medium_instance` đạt `mean_matched_IoU = 0.940`, `recall = 1.0`, đạt tối đa 32.0/32 điểm và không có cờ cảnh báo `REVIEW_IDENTICAL_GEOMETRY`. Scorecard ba tier tối đa **82**, không phải điểm cuối trên 100. Không tự ghi PASS/top 3/bonus; người phụ trách xác nhận theo tiêu chí lớp. Không đưa file ground truth vào fork.
