...
Nếu bạn **đã xem Summary tự đánh giá trên GitHub Actions hoặc tự chạy script**, ghi ngắn một kết quả liên quan lỗi vừa sửa (ví dụ task, metric trước/sau nếu có): Sau khi sửa và cập nhật các task, chạy kiểm tra tự đánh giá với bộ reference chính thức từ giáo viên cho kết quả: `medium_instance` đạt `mean_matched_IoU = 0.926`, `recall = 0.93`, đạt trọn vẹn 32.0/32 điểm; `hard_panoptic` đạt `PQ = 0.822` (30.0/30 điểm); `easy_semantic` đạt `mIoU = 0.679` (12.4/20 điểm). Tổng điểm ba tier đạt **74.4 / 82 điểm**, không có bất kỳ cờ cảnh báo vi phạm nào (`no review signals`). Không tự ghi PASS/top 3/bonus; người phụ trách xác nhận theo tiêu chí lớp. Không đưa file ground truth vào fork.
